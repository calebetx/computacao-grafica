...
# --- Supondo que os seus ficheiros de algoritmos estão em subpastas ---
...
# --- FUNÇÃO DE REFLEXÃO 3D CORRIGIDA ---
def reflexao3d(vertices, plano):
    """
    Reflete os vértices de um objeto 3D em relação a um plano usando coordenadas homogéneas.
    """
    ...
    if plano == 'plano_xy':
        ...
        return vertices # Se 'nenhuma' ou inválido, não faz nada

    vertices_homogeneos = np.c_[vertices, np.ones(vertices.shape[0])]
    vertices_transformados = vertices_homogeneos @ matriz_reflexao.T
...
class Canvas2D(OpenGLFrame):
    ...
    def redraw(self):
        ...
        if width > 0 and height > 0:
            draw_line_2d(-width / 2, 0, width / 2, 0)
            draw_line_2d(0, -height / 2, 0, height / 2)
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Visualizador de Transformações 2D e 3D")
    ...
    def criar_controles_2d(self):
        ...
        def _criar_controles_linha(parent, label1, key1, val1, label2, key2, val2, cmd):
            fr = ttk.LabelFrame(parent, text=label1.split(':')[0], padding=10)
            fr.pack(fill=tk.X, pady=5)
            
            ttk.Label(fr, text=f"{label1}:").pack(side=tk.LEFT)
            e1 = ttk.Entry(fr, width=7); e1.insert(0, val1); e1.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 5))
            self.entries_2d[key1] = e1
            
            ttk.Label(fr, text=f"{label2}:").pack(side=tk.LEFT)
            e2 = ttk.Entry(fr, width=7); e2.insert(0, val2); e2.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 5))
            self.entries_2d[key2] = e2
            
            ttk.Button(fr, text="Aplicar", command=cmd).pack(side=tk.LEFT)

        _criar_controles_linha(frame, "Translação dx", 'tx', "10.0", "dy", 'ty', "10.0", self.aplicar_translacao_2d)
        _criar_controles_linha(frame, "Escala sx", 'sx', "1.2", "sy", 'sy', "1.2", self.aplicar_escala_2d)
        _criar_controles_linha(frame, "Cisalhamento shx", 'shx', "0.2", "shy", 'shy', "0.0", self.aplicar_cisalhamento_2d)
    ...
    def resetar_3d(self):
        ...
        # Limpa e repõe os valores nos campos de entrada
        for key, entry in self.entries_3d.items():
            if key in ['tx', 'ty', 'tz', 'rx', 'ry', 'rz', 'sh_xy', 'sh_xz', 'sh_yz']:
                entry.delete(0, tk.END); entry.insert(0, "0.0")
            elif key in ['sx', 'sy', 'sz']:
                entry.delete(0, tk.END); entry.insert(0, "1.0")
            elif key == 'reflexao_plano':
                entry.set('nenhuma')
